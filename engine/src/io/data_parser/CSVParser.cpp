...
std::unique_ptr<ral::frame::BlazingTable> csv_parser::parse_batch(ral::io::data_handle handle,
	const Schema & schema,
	...
	if(column_indices.size() > 0) {
		...
		cudf::io::arrow_io_source arrow_source{file};
		auto source = dynamic_cast<arrow::io::ReadableFile *>(file.get()) != nullptr
						  ? cudf::io::source_info{handle.uri.getPath().toString()}
						  : cudf::io::source_info{&arrow_source};
		cudf::io::csv_reader_options args = getCsvReaderOptions(args_map, source);
		...
		// Overrride `_byte_range_size` param to read first `max_bytes_chunk_read` bytes (note: always reads complete
		// rows)
...
void csv_parser::parse_schema(ral::io::data_handle handle, ral::io::Schema & schema) {
...
std::unique_ptr<ral::frame::BlazingTable> csv_parser::get_metadata(
	std::vector<ral::io::data_handle> handles, int offset, std::map<std::string, std::string> args_map) {
