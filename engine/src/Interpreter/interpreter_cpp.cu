...
struct allocate_device_scalar {
	...
	rmm::device_buffer operator()(cudf::scalar & s, cudaStream_t stream = 0) {
		...
		ScalarDeviceType h_scalar{
			typed_scalar_ptr->type(), typed_scalar_ptr->data(), typed_scalar_ptr->validity_data()};

		CUDF_CUDA_TRY(cudaMemcpyAsync(ret.data(), &h_scalar, sizeof(ScalarDeviceType), cudaMemcpyDefault, stream));
	...
	rmm::device_buffer operator()(cudf::scalar & s, cudaStream_t stream = 0) {
		...
		auto typed_scalar_ptr = static_cast<ScalarType *>(&s);
		ScalarDeviceType h_scalar{typed_scalar_ptr->type(),
			typed_scalar_ptr->data(),
			typed_scalar_ptr->validity_data(),
			typed_scalar_ptr->size()};
...
public:
	expr_to_plan_visitor(const std::map<column_index_type, column_index_type> & expr_idx_to_col_idx_map,
		...
		: expr_idx_to_col_idx_map{expr_idx_to_col_idx_map}, start_processing_position{start_processing_position},
		  left_inputs{left_inputs}, right_inputs{right_inputs}, outputs{outputs}, operators{operators},
		  left_scalars{left_scalars}, right_scalars{right_scalars}, processing_space_free_(512, true) {
	...
	void visit(const ral::parser::operator_node & node) override {
		...
		if(is_binary_operator(operation)) {
			...
			if(left_operand->type == ral::parser::node_type::LITERAL &&
				right_operand->type == ral::parser::node_type::LITERAL) {
			...
			const ral::parser::node * left_operand = node.children[0].get();
			RAL_EXPECTS(
				left_operand->type != ral::parser::node_type::LITERAL, "Unary operations on literals is not supported");
...
private:
	...
	std::vector<bool>
		processing_space_free_;	 // A place to store whether or not a processing space is occupied at any point in time
...
void perform_interpreter_operation(cudf::mutable_table_view & out_table,
	...
	RAL_EXPECTS(std::max(std::max(*max_left_it, *max_right_it), *max_out_it) < 64,
		"Interops does not support plans with an input or output index greater than 63");
	...
	size_t temp_valids_out_size =
		min_grid_size * block_size * final_output_positions.size() * sizeof(cudf::bitmask_type);
	...
	for(size_t i = 0; i < left_scalars.size(); i++) {
		left_device_scalars_ptrs.push_back(
			left_scalars[i] ? std::move(cudf::type_dispatcher(
								  left_scalars[i]->type(), allocate_device_scalar{}, *(left_scalars[i])))
							: rmm::device_buffer{});
		left_device_scalars_raw.push_back(
			static_cast<cudf::detail::scalar_device_view_base *>(left_device_scalars_ptrs.back().data()));

		right_device_scalars_ptrs.push_back(
			right_scalars[i] ? std::move(cudf::type_dispatcher(
								   right_scalars[i]->type(), allocate_device_scalar{}, *(right_scalars[i])))
							 : rmm::device_buffer{});
		right_device_scalars_raw.push_back(
			static_cast<cudf::detail::scalar_device_view_base *>(right_device_scalars_ptrs.back().data()));
	}
	...
	setup_rand_kernel<<<min_grid_size, block_size, shared_memory_per_thread * block_size, stream>>>(
		states.data().get(), seed);
	if(operation_num_rows == 0) {
		operation_num_rows = table.num_rows();
	}
	transformKernel<<<min_grid_size, block_size, shared_memory_per_thread * block_size, stream>>>(
		op, operation_num_rows, states.data().get());
